/**
 * API Server — Hạt Điều Vàng
 * Chạy: node server/api.js
 * Lắng nghe POST /send-email → gửi qua Resend
 */
import http   from "http";
import { Resend } from "resend";

const PORT        = 3001;
const RESEND_KEY  = process.env.RESEND_API_KEY  || "re_MqFohr7a_2mgrcwtJHBzwGYvAsvkwct5d";
const OWNER_EMAIL = process.env.OWNER_EMAIL      || "hungtran30415@gmail.com";
...
<div style="max-width:540px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.1)">
  ...
  <div style="padding:20px 28px;border-bottom:2px solid #f0e0c8">
    <h2 style="margin:0 0 10px;color:#4a2c0a;font-size:.95rem;text-transform:uppercase;letter-spacing:.5px">Khách hàng</h2>
  ...
  <div style="padding:20px 28px">
    <h2 style="margin:0 0 10px;color:#4a2c0a;font-size:.95rem;text-transform:uppercase;letter-spacing:.5px">Sản phẩm</h2>
...
const server = http.createServer(async (req, res) => {
  // CORS — cho phép trang web gọi từ file://
  ...
  if (req.method === "POST" && req.url === "/send-email") {
    ...
    req.on("end", async () => {
      try {
        ...
        console.log(`[${new Date().toLocaleTimeString()}] Email gửi: ${data.id} — ${order.id}`);
...
server.listen(PORT, () => {
  console.log(`🌰 Hạt Điều Email Server đang chạy: http://localhost:${PORT}`);
  console.log(`   Gửi đến: ${OWNER_EMAIL}`);
});
